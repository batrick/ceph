...
To reduce load on the metadata pool, the Manager can keep a short-lived in-memory
cache of formatted metrics returned by ``fs snapshot mirror status``. Caching is
enabled by default (``snapshot_mirror_metrics_cache_enabled``). When disabled,
each query reads omap directly. When enabled, the default time-to-live is 15
seconds (``snapshot_mirror_metrics_cache_ttl``). Repeated queries within the TTL
may be served without reading omap again. After the cache expires, the next
query reloads metrics from omap.
