...
will be recovered by SQLite and the in-progress transaction rolled back.
...
database. For this reason, it is necessary to utilize RADOS blocklisting
functionality in any hardened application. For example, in the ``ceph-mgr``,
the RADOS connection used for libcephsqlite is blocklisted anytime there is a
``ceph-mgr`` failover. A zombie ``ceph-mgr`` will not be able to perform writes
when it recovers from the network partition.
