...
""", wait=wait)
    ...
    def test_ephemeral_pin_dist_override(self):
        ...
        subtrees = self._get_subtrees(status=self.status)
        for s in subtrees:
            path = s['dir']['path']
            if path == '/tree':
    ...
    def test_ephemeral_pin_dist_override_before(self):
        ...
        count = 10
        self._setup_tree(count=count, export=0)
        test = [("/tree", 0)]
        ...
        self._wait_subtrees(test, timeout=60, status=self.status)
    ...
    def test_ephemeral_pin_dist_override_after(self):
        ...
        subtrees = self._wait_distributed_subtrees(10, status=self.status)
        victim = None
        test = []
        for s in subtrees:
            if s['auth_first'] == 0 and s['distributed_ephemeral_pin']:
                victim = s['dir']['path']
            elif s['auth_first'] == 1 and s['distributed_ephemeral_pin']:
                test.append((s['dir']['path'], 1))
    ...
    def test_ephemeral_pin_dist_failover(self):
        ...
        self._setup_tree(distributed=True, export=0)
        subtrees = self._wait_distributed_subtrees(100, status=self.status)
    ...
    def test_ephemeral_pin_distribution(self):
        ...
        rank0_distributed_subtree_ratio = len(self.get_distributed_auth_subtrees(self.status, 0))/len(self.get_auth_subtrees(self.status, 0))
        self.assertGreaterEqual(rank0_distributed_subtree_ratio, 0.2)

        rank1_distributed_subtree_ratio = len(self.get_distributed_auth_subtrees(self.status, 1))/len(self.get_auth_subtrees(self.status, 1))
        self.assertGreaterEqual(rank1_distributed_subtree_ratio, 0.2)

        rank2_distributed_subtree_ratio = len(self.get_distributed_auth_subtrees(self.status, 2))/len(self.get_auth_subtrees(self.status, 2))
    ...
    def test_ephemeral_random(self):
        ...
        """
        self._setup_tree(random=1.0, export=0)
    ...
    def test_ephemeral_random_max(self):
        ...
        subtrees = self._wait_random_subtrees(int(r*count*.85), status=self.status)
        ...
        subtrees = self._get_subtrees(status=self.status)
        self.assertLessEqual(len(subtrees), int(r*count*1.25)+int(.01*count*1.25))
    ...
    def test_ephemeral_random_pin_override_before(self):
        ...
        self._wait_subtrees([("/tree/pin", 1)], status=self.status, rank=1)
    ...
    def test_ephemeral_random_pin_override_after(self):
        ...
        count = 10
        self._setup_tree(count=0, random=1.0, export=0)
        self._setup_tree(path="tree/pin", count=count)
        self._wait_random_subtrees(count+1, status=self.status)
        self.mount_a.setfattr(f"tree/pin", "ceph.dir.pin", "1")
        self._wait_subtrees([("/tree", 0), ("/tree/pin", 1)], status=self.status, rank=1)
    ...
    def test_ephemeral_randomness(self):
        ...
        self._setup_tree(count=count, random=r, export=0)
        subtrees = self._wait_random_subtrees(int(r*count*.85), status=self.status)
        self.assertLessEqual(len(subtrees), int(r*count*1.15))
    ...
    def test_ephemeral_random_cache_drop(self):
        ...
        count = 100
        self._setup_tree(count=count, random=1.0, export=0)
        subtrees = self._wait_random_subtrees(count, status=self.status)
        self.mount_a.umount_wait() # release all caps
        def _drop():
            self.fs.ranks_tell(["cache", "drop"], status=self.status)
        self._wait_subtrees([], status=self.status, action=_drop)
    ...
    def test_ephemeral_pin_grow_mds(self):
        ...
        subtrees_old = self.get_ephemerally_pinned_auth_subtrees(self.status, 0) + self.get_ephemerally_pinned_auth_subtrees(self.status, 1)
        self.fs.set_max_mds(3)
        self.status = self.fs.wait_for_daemons()
        ...
        subtrees_new = self.get_ephemerally_pinned_auth_subtrees(self.status, 0) + self.get_ephemerally_pinned_auth_subtrees(self.status, 1) + self.get_ephemerally_pinned_auth_subtrees(self.status, 2) 
        count = 0
        ...
        self.assertLessEqual((count/len(subtrees_old)), 0.5)
    ...
    def test_ephemeral_pin_shrink_mds(self):
        ...
        subtrees_old = self.get_ephemerally_pinned_auth_subtrees(self.status, 0) + self.get_ephemerally_pinned_auth_subtrees(self.status, 1) + self.get_ephemerally_pinned_auth_subtrees(self.status, 2)
        self.fs.set_max_mds(2)
        self.status = self.fs.wait_for_daemons()
        time.sleep(30)

        subtrees_new = self.get_ephemerally_pinned_auth_subtrees(self.status, 0) + self.get_ephemerally_pinned_auth_subtrees(self.status, 1)
        count = 0
        for old_subtree in subtrees_old:
            for new_subtree in subtrees_new:
                if (old_subtree['dir']['path'] == new_subtree['dir']['path']) and (old_subtree['auth_first'] != new_subtree['auth_first']):
        ...
        self.assertLessEqual((count/len(subtrees_old)), (1.0/3.0/2.0 + 1.0/3.0/2.0 + 1.0/3.0)) # aka .66
