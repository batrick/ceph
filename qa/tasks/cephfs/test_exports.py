...
""", wait=wait)
    ...
    def test_ephemeral_pin_dist_override_after(self):
        ...
        self._wait_subtrees(test, status=self.status, rank="all")
    ...
    def test_ephemeral_pin_distribution(self):
        ...
        self._setup_tree(distributed=True)
        subtrees = self._wait_distributed_subtrees(1000, status=self.status, rank="all")
        nsubtrees = len(subtrees)

        # Check if distribution is uniform
        rank0 = filter(lambda x: x['auth_first'] == 0, subtrees)
        rank1 = filter(lambda x: x['auth_first'] == 1, subtrees)
        rank2 = filter(lambda x: x['auth_first'] == 2, subtrees)
