...
vc.disconnect()
    ...
    def test_put_object(self):
        ...
        temp_bin_path = '/tmp/' + obj_name + '.bin'
        self.fs.client_remote.run(args=[
            'sudo', os.path.join(self.fs._prefix, 'rados'), '-p', pool_name,
            'get', obj_name, temp_bin_path
        ])
    ...
    def test_get_object(self):
        ...
        with open(temp_bin_path, 'wb') as f:
            read_data = f.write(obj_data)
        self.fs.client_remote.run(args=[
            'sudo', os.path.join(self.fs._prefix, 'rados'), '-p', pool_name,
            'put', obj_name, temp_bin_path
        ])
