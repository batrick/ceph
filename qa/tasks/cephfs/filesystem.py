...
class MDSCluster(CephCluster):
    ...
    def __init__(self, ctx):
        ...
        if hasattr(self._ctx, "daemons"):
            ...
            mds_ids = list(misc.all_roles_of_type(self._ctx.cluster, 'mds'))
            self.mds_daemons = dict([(mds_id, self._ctx.daemons.get_daemon('mds', mds_id)) for mds_id in mds_ids])
    ...
    def get_mds_hostnames(self):
        result = set()
        for mds_id in misc.all_roles_of_type(self._ctx.cluster, 'mds'):
...
class Filesystem(MDSCluster):
    ...
    def dencoder(self, obj_type, obj_blob):
        ...
        p = self.mon_manager.controller.sh(args, stdin=BytesIO(obj_blob), stdout=BytesIO())
    ...
    def radosmo(self, *args, stdout=BytesIO(), **kwargs):
        ...
        return self.radosm(stdout=stdout).stdout.getvalue()

    def put_metadata_object_raw(self, object_id, blob):
        """
        Save an object to the metadata pool
        """

        return self.radosm(['put', object_id], stdin=BytesIO(blob))
    ...
    def get_metadata_object_raw(self, object_id):
        ...
        return self.radosmo(['get', object_id])
    ...
    def _enumerate_data_objects(self, ino, size):
        ...
        exist_objects = self.rados(["ls"], pool=self.get_data_pool_name(), stdout=BytesIO()).stdout.getvalue().split("\n")
    ...
    def list_dirfrag(self, dir_ino):
        ...
        try:
            key_list_str = self.radosmo(["listomapkeys", dirfrag_obj_name])
        except CommandFailedError as e:
            log.error(e.__str__())
    ...
    def get_meta_of_fs_file(self, dir_ino, obj_name, out):
        ...
        dirfrag_obj_name = "{0:x}.00000000".format(dir_ino)
        try:
            return self.radosmo(["getomapval", dirfrag_obj_name, obj_name+"_head", out])
        except CommandFailedError as e:
            log.error(e.__str__())
    ...
    def erase_metadata_objects(self, prefix):
        ...
        """
        all_objects = self.radosmo(["ls"]).split("\n")
