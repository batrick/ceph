...
class XFSTestsDev(CephFSTestCase):
    ...
    def write_local_config(self):
        ...
        def get_a_mon_socket_addr():
            from json import loads

            mon_status = loads(self.fs.mon_manager.raw_cluster_cmd(
                'mon_status'))
            mon_socket_addr = mon_status['monmap']['mons'][0]['public_addrs']\
                ['addrvec'][1]['addr']
            return mon_socket_addr

        mon_socket_addr = get_a_mon_socket_addr()
        self.test_dev = mon_socket_addr + ':/' + self.test_dirname
        self.scratch_dev = mon_socket_addr + ':/' + self.scratch_dirname
