...
class QuiesceTestCase(CephFSTestCase):
    ...
    def _wait_for_quiesce_complete(self, reqid, rank=0, path=None, timeout=120):
        if path is None:
            path = self.subvolume
        op = None
    ...
    def _verify_quiesce(self, rank=0, root=None, splitauth=None):
        if root is None:
            root = self.subvolume
        ...
        root_inode = self.fs.read_cache(root, depth=0, rank=rank)[0]
        ...
        cache = self.fs.read_cache(root, rank=rank, path=f"/tmp/mds.{rank}-cache")
        visited = set()
        locks_expected = set([
          "iquiesce",
        ])
        ...
        try:
            ...
            for inode in cache:
                ...
                if not auth and not splitauth:
                    continue
                inos.add(ino)
            self.assertEqual(sorted(list(inos)), sorted(list(quiesce_inode_ops.keys())))
            for inode in cache:
                ino = inode['ino']
                mode = inode['mode']
                auth = inode['is_auth']
...
class TestQuiesce(QuiesceTestCase):
    ...
    def test_quiesce_find(self):
        ...
        # now try `find`
        self.mount_a.run_shell_payload('find -printf ""', timeout=120)
