...
function sqlite {
  ...
  sqlite3 -cmd '.bail on' -cmd '.output /dev/null' -cmd '.load libcephsqlite.so' -cmd 'pragma journal_mode = PERSIST' -cmd ".open file:///$pool:$ns/baz.db?vfs=ceph" -cmd '.output stdout' -cmd 'PRAGMA busy_timeout = 5000' <<<"$a" &
  if [ "$background" != b ]; then
    wait
  fi
}
