...
ceph osd pool create "$pool" || true

#rados --namespace="$ns" purge "$pool" --yes-i-really-really-mean-it
...
function sqlite {
  ...
  a=$(cat)
  printf "%s" "$a" >&2
  # We're doing job control gymnastics here to make sure that sqlite3 is what
  # is the main process (i.e. the process group leader) in the background, not
  # a bash function or job pipeline.
