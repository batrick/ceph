...
protected:
  int db_open()
  {
    static const char SQL[] =
      ...
      "CREATE TABLE memory.perf (i INTEGER PRIMARY KEY, v TEXT);"
      "CREATE VIEW memory.p AS"
      "    SELECT perf.i, J.*"
      "    FROM memory.perf, json_tree(memory.perf.v) AS J;"
      "INSERT INTO memory.perf (v)"
      "    VALUES (ceph_perf());"
      ;
...
TEST_F(CephSQLiteTest, WALTransactionSync) {
  static const char SQL[] =
    ...
    "    FROM memory.p AS a, memory.p AS b"
    "    WHERE a.i = ? AND"
    "          b.i = ? AND"
