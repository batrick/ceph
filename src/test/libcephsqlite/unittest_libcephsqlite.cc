...
public:
  static void SetUpTestSuite() {
    ...
    cluster.pool_delete("cephsqlite");
    ASSERT_EQ(0, cluster.pool_create("cephsqlite"));
    cluster.shutdown();
    sleep(5);
  }
...
protected:
  int db_open()
  {
    ...
    auto&& name = get_name();
    sqlcatch(sqlite3_open_v2(name.c_str(), &db, SQLITE_OPEN_CREATE|SQLITE_OPEN_READWRITE, "ceph"));
...
out:
  ...
  virtual std::string get_name() const {
    auto name = fmt::format("cephsqlite:/{}.db", uuid.to_string());
    return name;
  }
...
TEST_F(CephSQLiteTest, InsertRate) {
...

  {
    ...
    std::cout << "transactions per second: " << diff.count()/count << std::endl;
  }
...
TEST_F(CephSQLiteTest, InsertMassive) {
  static const char SQL[] =
    ...
    "ATTACH DATABASE 'file:local.db?vfs=unix-none' AS local;"
    "CREATE TABLE local.bar (a BLOB);"
    "WITH RECURSIVE c(x) AS"
    "  ("
    "   VALUES(1)"
    "  UNION ALL"
    ...
    "   FROM c"
    "  )"
    "INSERT INTO local.bar (a)"
    "  SELECT RANDOMBLOB(1<<20)"
    "  FROM c"
    "  LIMIT 128;"
    "SELECT a FROM local.bar;"
    "INSERT INTO foo (a)"
    "  SELECT a FROM local.bar;"
    "SELECT a FROM foo;"
    "DROP TABLE local.bar;"
    ;
