...
bool MDSMonitor::preprocess_beacon(MonOpRequestRef op)
{
  ...
  if (m->get_fs().size()) {
    ...
    if (info.join_fs != fscid) {
      dout(10) << __func__ << " standby mds_join_fs changed to " << fscid
               << " (" << m->get_fs() << ")" << dendl;
      return false;
    }
  } else {
    if (info.join_fs != FS_CLUSTER_ID_NONE) {
 ...
 */
bool MDSMonitor::replace_gid(FSMap &fsmap, mds_gid_t gid, const mds_info_t* rep_info, bool *osd_propose)
{
  ...
  if (info.is_frozen()) {
    ...
    fail_mds_gid(fsmap, gid);
    return true;
  } else if (rank >= 0 && rep_info && !fs->mds_map.test_flag(CEPH_MDSMAP_NOT_JOINABLE)) {
    // are we in?
...
bool MDSMonitor::check_ranks(FSMap& fsmap, bool* propose_osdmap)
{
  ...
  for (auto it = last_beacon.begin(); it != last_beacon.end(); ) {
    ...
    if (since_last.count() >= g_conf()->mds_beacon_grace) {
      ...
      if (osdmap_writeable && may_replace) {
        auto fscid = fsmap.mds_roles.at(gid);
        auto rep_info = fsmap.find_replacement_for({fscid, info.rank});
        mon->clog->info() << "MDS " << info.human_name()
                          << " is dead or otherwise unavailable, dropping it";
        do_propose |= replace_gid(fsmap, gid, rep_info, propose_osdmap);
      }
    }
  ...
  for (auto& [fscid, fs] : fsmap.filesystems) {
    if (!fs->mds_map.test_flag(CEPH_MDSMAP_NOT_JOINABLE) &&
      ...
      for (const auto& [gid, info] : fs->mds_map.get_mds_info()) {
        const auto join_fs = info.join_fs;
        if (join_fs == fscid)
          continue;
        const auto rank = info.rank;
        const auto state = info.state;
        ...
        if (join_fs == FS_CLUSTER_ID_NONE) {
          better_affinity = (rep_info->join_fs == fscid);
        } else {
          better_affinity = (rep_info->join_fs == fscid) ||
                            (rep_info->join_fs == FS_CLUSTER_ID_NONE);
        }
        if (better_affinity) {
          if (state == MDSMap::STATE_STANDBY_REPLAY) {
...
void MDSMonitor::tick()
{
  ...
  auto mds_beacon_grace = g_conf().get_val<double>("mds_beacon_grace");
  ...
  do_propose |= check_ranks(pending, &propose_osdmap);
