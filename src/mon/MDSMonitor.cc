...
void MDSMonitor::encode_pending(MonitorDBStore::TransactionRef t)
{
  ...
  // apply to paxos
  assert(get_last_committed() + 1 == fsmap.epoch);
  bufferlist fsmap_bl;
  fsmap.encode(fsmap_bl, mon->get_quorum_con_features());

  /* put everything in the transaction */
  put_version(t, fsmap.epoch, fsmap_bl);
  put_last_committed(t, fsmap.epoch);
  ...
  fsmap.get_health_checks(&new_checks);
...
bool MDSMonitor::prepare_beacon(MonOpRequestRef op)
{
  ...
  if (state == MDSMap::STATE_BOOT) {
    ...
    if (g_conf->mds_enforce_unique_name) {
      ...
      while (mds_gid_t existing = fsmap.find_mds_gid_by_name(m->get_name())) {
        if (!mon->osdmon()->is_writeable()) {
          mon->osdmon()->wait_for_writeable(op, new C_RetryMessage(this, op));
          return false;
        }
        const MDSMap::mds_info_t &existing_info =
          fsmap.get_info_gid(existing);
    ...
    if (fsmap.mds_roles.count(gid) == 0) {
    ...
    if (!info.standby_for_name.empty()) {
      ...
      if (leaderinfo && (leaderinfo->rank >= 0)) {
        const auto &fscid = fsmap.mds_roles.at(leaderinfo->global_id);
        const auto &fs = fsmap.get_filesystem(fscid);

        fsmap.modify_daemon(gid, [fscid, leaderinfo](
              MDSMap::mds_info_t *info) {
    ...
    // new incompat?
    if (!fsmap.compat.writeable(m->get_compat())) {
      dout(10) << " fsmap " << fsmap.compat
               << " can't write to new mds' " << m->get_compat()
	       << ", updating fsmap and killing old mds's"
	       << dendl;
      fsmap.update_compat(m->get_compat());
    }
    ...
    if (info.standby_replay != m->get_standby_replay()) {
      fsmap.modify_daemon(info.global_id, [&m](
            MDSMap::mds_info_t *i)
        {
    ...
    if (state == MDSMap::STATE_STOPPED) {
      ...
      request_proposal(mon->osdmon());
      fsmap.damaged(gid, blacklist_epoch);
      ...
      fsmap.modify_daemon(gid, [state, seq](MDSMap::mds_info_t *info) {
        info->state = state;
        info->state_seq = seq;
      });
    }
...
bool MDSMonitor::prepare_offload_targets(MonOpRequestRef op)
{
  ...
  if (get_fsmap().gid_has_rank(gid)) {
    dout(10) << "prepare_offload_targets " << gid << " " << m->targets << dendl;
    get_pending_fsmap_writeable().update_export_targets(gid, m->targets);
  } else {
...
void MDSMonitor::_updated(MonOpRequestRef op)
{
  ...
  if (m->get_state() == MDSMap::STATE_STOPPED) {
    ...
    MDSMap null_map;
    null_map.epoch = get_fsmap().epoch;
    null_map.compat = get_fsmap().compat;
    mon->send_reply(op, new MMDSMap(mon->monmap->fsid, &null_map));
  } else {
...
bool MDSMonitor::prepare_command(MonOpRequestRef op)
{
  ...
  for (const auto &h : handlers) {
    if (h->can_handle(prefix)) {
      ...
      r = h->handle(mon, get_pending_fsmap_writeable(), op, cmdmap, ss);
      if (batched_propose) {
        paxos->unplug();
      }
...
bool MDSMonitor::maybe_resize_cluster(std::shared_ptr<Filesystem> &fs)
{
  ...
  if (in < max && !fs->mds_map.test_flag(CEPH_MDSMAP_NOT_JOINABLE)) {
    ...
    mds_gid_t newgid = get_fsmap().find_replacement_for({fs->fscid, mds},
                         name, g_conf->mon_force_standby_active);
    if (newgid == MDS_GID_NONE) {
      return false;
    }

    const auto &new_info = get_fsmap().get_info_gid(newgid);
    ...
    mon->clog->info() << new_info.human_name() << " assigned to filesystem "
		      << fs->mds_map.fs_name << " as rank "
                      << mds << " (now has "
		      << fs->mds_map.get_num_in_mds() + 1 << " ranks)";
    get_pending_fsmap_writeable().promote(newgid, fs, mds);
    return true;
  } else if (in > max) {
    ...
    if (fs->mds_map.is_active(target)) {
      ...
      get_pending_fsmap_writeable().modify_daemon(info.global_id,
				  [] (MDSMap::mds_info_t *info) {
    ...
    for (const auto &gid : standby_gids) {
      ...
      if (info.standby_for_rank >= 0) {
        ...
        if (target_role.fscid != FS_CLUSTER_ID_NONE) {
          auto fs = fsmap.get_filesystem(target_role.fscid);
