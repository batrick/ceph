...
bool Locker::_rdlock_kick(SimpleLock *lock, bool as_anon)
{
  ...
  if (lock->is_stable()) {
	...
	mds->send_message_mds(MLock::ref(new MLock(lock, LOCK_AC_REQRDLOCK, mds->get_nodeid()), auth), false);
      }
      return false;
    }
  }
...
bool Locker::wrlock_start(SimpleLock *lock, MDRequestRef& mut, bool nowait)
{
  ...
  while (1) {
	...
	dout(10) << "requesting scatter from auth on "
		 << *lock << " on " << *lock->get_parent() << dendl;
	mds->send_message_mds(MLock::ref(new MLock(lock, LOCK_AC_REQSCATTER, mds->get_nodeid()), auth), false);
      }
      break;
    }
  }
