...
void CInode::pop_and_dirty_projected_inode(LogSegment *ls, const MutationRef& mut)
{
  ...
  bool new_export_pin = get_inode()->export_pin != front.inode->export_pin;
  reset_inode(front.inode);
  if (new_export_pin)
			      ...
			      bufferlist& snap_blob, __u8 struct_v)
{
  ...
  inode_ptr _inode;
  if (inode && inode->version == 0)
    _inode = const_inode_cast(inode);
  else {
    _inode = allocate_inode(*inode);
    reset_inode(_inode);
  }
  decode(*_inode, bl);
...
void CInode::decode_lock_ipolicy(bufferlist::const_iterator& p)
{
  ...
  DECODE_FINISH(p);
  mds_rank_t old_pin = get_inode()->export_pin;
  reset_inode(_inode);
  maybe_export_pin(old_pin != get_inode()->export_pin);
}
...
void CInode::_decode_base(bufferlist::const_iterator& p)
{
  ...
  {
    auto _inode = get_version() == 0 ? _get_inode() : allocate_inode();
    decode(*_inode, p);
    if (_inode != get_inode())
      reset_inode(_inode);
  }
...
void InodeStore::generate_test_instances(std::list<InodeStore*> &ls)
{
  ...
  const_inode_cast(populated->inode)->ino = 0xdeadbeef;
