				  ...
				  file_layout_t *layout)
{
  ...
  if (req->get_data().length()) {
    ...
    for (const auto &p : xattrs) {
      ...
      in->xattrs.emplace(std::piecewise_construct, std::forward_as_tuple(p.first), std::forward_as_tuple(p.second));
    }
  }
...
void Server::handle_client_setxattr(MDRequestRef& mdr)
{
  ...
  if ((flags & CEPH_XATTR_REMOVE)) {
    ...
    px.emplace(std::piecewise_construct, std::forward_as_tuple(mempool::mds_co::string(name)), std::forward_as_tuple(b));
  }
