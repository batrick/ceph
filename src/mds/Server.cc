			     ...
			     interval_set<inodeno_t>& inos, version_t piv)
{
  ...
  if (session->get_state_seq() != state_seq) {
    ...
    session->connection->send_message2(reply);
    if (mdcache->is_readonly())
      session->connection->send_message2(MClientSession::factory::build(CEPH_SESSION_FORCE_RO));
...
public:
  CF_MDS_MDRContextFactory(MDCache *cache, MDRequestRef &mdr) : mdr(mdr) {}
