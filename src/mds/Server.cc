...
public:
  ...
  C_MDS_session_finish(Server *srv, Session *se, uint64_t sseq, bool s, version_t mv, interval_set<inodeno_t>& i, version_t iv,
    ...
    ServerLogContext(srv), session(se), state_seq(sseq), open(s), cmapv(mv), inos(i), inotablev(iv), purge_inos(_purge_inos), ls(_ls), fin(fin_){}
...
void Server::_session_logged(Session *session, uint64_t state_seq, bool open, version_t pv,
			     ...
			     interval_set<inodeno_t>& purge_inos, LogSegment *ls)
{
  dout(10) << "_session_logged " << session->info.inst
	   << " state_seq " << state_seq
	   << " " << (open ? "open":"close")
	   << " " << pv
	   << " purge_inos : " << purge_inos
	   << " seq : " << ls->seq << dendl;
