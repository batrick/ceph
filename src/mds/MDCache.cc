...
void MDCache::dispatch_quiesce_inode(const MDRequestRef& mdr)
{
  ...
  /* XXX what about replicas for new dentries after quiesce is complete? (handled now by CDir::link_inode_work) */
  /* TODO block imports: add flag to path_traverse to fail if discover is necessary when the parent inode is quiesced. The abort import. */
