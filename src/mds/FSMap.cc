...
{
  std::string ns_err;
  fs_cluster_id_t fscid = strict_strtol(ns_str.data(), 10, &ns_err);
    ...
    std::ostream &ss) const
{
  ...
  mds_rank_t rank;
  std::string err;
  std::string_view rank_str = role_str.substr(rank_pos);
  long rank_i = strict_strtol(rank_str.data(), 10, &err);
