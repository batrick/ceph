...
class MDSRank {
  ...
  public:
    ...
    SessionRef get_session(const ceph::cref_t<Message>& m);
  ...
  protected:
    ...
    list<ceph::cref_t<Message>> waiting_for_nolaggy;
    MDSContext::que finished_queue;
    // Dispatch, retry, queues
    int dispatch_depth;
    void inc_dispatch_depth() { ++dispatch_depth; }
    void dec_dispatch_depth() { --dispatch_depth; }
    void retry_dispatch(const ceph::cref_t<Message>& m);
    bool handle_deferrable_message(const ceph::cref_t<Message>& m);
    void _advance_queues();
    bool _dispatch(const ceph::cref_t<Message>& m, bool new_msg);

    ceph::heartbeat_handle_d *hb;  // Heartbeat for threads using mds_lock

    bool is_stale_message(const ceph::cref_t<Message>& m) const;
  ...
  public:
    ...
    void send_message_mds(const ceph::ref_t<Message>& m, mds_rank_t mds);
    void forward_message_mds(const ceph::cref_t<MClientRequest>& req, mds_rank_t mds);
    void send_message_client_counted(const ceph::ref_t<Message>& m, client_t client);
    void send_message_client_counted(const ceph::ref_t<Message>& m, const SessionRef& session);
    void send_message_client_counted(const ceph::ref_t<Message>& m, const ConnectionRef& connection);
    void send_message_client(const ceph::ref_t<Message>& m, const SessionRef& session);
    void send_message(const ceph::ref_t<Message>& m, const ConnectionRef& c);
...
public:
  C_MDS_RetryMessage(MDSRank *mds, const ceph::cref_t<Message>& m)
    : MDSInternalContext(mds), m(m) {}
  void finish(int r) override {
    get_mds()->retry_dispatch(m);
  }
protected:
  ceph::cref_t<Message> m;
};

class CF_MDS_RetryMessageFactory : public MDSContextFactory {
public:
  CF_MDS_RetryMessageFactory(MDSRank *mds, const ceph::cref_t<Message>& m)
    : mds(mds), m(m) {}
...
public:
  ...
  void handle_mds_map(const ceph::cref_t<MMDSMap>& m, const MDSMap &oldmap);
  void handle_osd_map();
  ...
  void evict_clients(const SessionFilter &filter, const ceph::cref_t<MCommand>& m);

  // Call into me from MDS::ms_dispatch
  bool ms_dispatch(const ceph::cref_t<Message>& m);
