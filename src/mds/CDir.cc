...
void CDir::merge(const std::vector<CDir*>& subs, MDSContext::vec& waiters, bool replay)
{
  ...
  for (const auto& dir : subs) {
    ...
    if (dir->get_version() > _fnode->version)
      _fnode->version = dir->get_version();
    ...
    if (!dn || undef_inode) {
      ...
      if (!in || undef_inode) {
        if (undef_inode && in)
          ...
          in = new CInode(mdcache, first, last, CInode::NULL_INODE);
        
        in->reset_inode(inode_data.inode);
        in->reset_xattrs(inode_data.xattrs);
        // symlink?
        if (in->is_symlink()) 
          in->symlink = inode_data.symlink;
        
        in->dirfragtree.swap(inode_data.dirfragtree);
	in->reset_old_inodes(inode_data.old_inodes);
