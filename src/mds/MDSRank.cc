...
private:
  ...
  void recall_client_state() {
    ...
    if ((throttled || count > 0) && duration < recall_timeout) {
      ...
      } else if (duration > recall_timeout) {
        delete gather;
        return handle_recall_client_state(-ETIMEDOUT);
      } else {
        double remaining = recall_timeout-duration;
  ...
  void finish(int r) override {
    ...
    auto d = std::chrono::duration<double>(clock::now()-recall_start);
