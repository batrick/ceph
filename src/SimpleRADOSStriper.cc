...
int SimpleRADOSStriper::setmeta(uint64_t s, bool update_size)
{
  d(5) << s << " " << update_size << dendl;

  d(10) << " allocated: " << allocated
        << " size: " << size
        << " version: " << version
        << dendl;
  ...
  if (update_size) {
    op.setxattr(XATTR_SIZE, uint2bl(s));
    do_op = true;
    d(15) << " updating size to " << s << dendl;
  }
  if (do_op) {
    op.setxattr(XATTR_VERSION, uint2bl(version+1));
    d(15) << " updating version to " << (version+1) << dendl;
