...
int SimpleRADOSStriper::create()
{
  ...
  batch_op.getxattr(XATTR_VERSION, nullptr, nullptr);
  batch_op.getxattr(XATTR_EXCL, nullptr, nullptr);
  batch_op.getxattr(XATTR_STATE, nullptr, nullptr);
  batch_op.getxattr(XATTR_WRITE_EPOCH, nullptr, nullptr);
  batch_op.getxattr(XATTR_SIZE, nullptr, nullptr);
  batch_op.getxattr(XATTR_ALLOCATED, nullptr, nullptr);
  batch_op.getxattr(XATTR_LAYOUT_STRIPE_UNIT, nullptr, nullptr);
  batch_op.getxattr(XATTR_LAYOUT_STRIPE_COUNT, nullptr, nullptr);
  batch_op.getxattr(XATTR_LAYOUT_OBJECT_SIZE, nullptr, nullptr);

  bufferlist out_bl;
  int batch_rc = ioctx.operate(ext.soid, &batch_op, &out_bl);
  if (batch_rc == 0) {
    // All xattrs exist, fast path out
    return 0;
  } else if (batch_rc < 0 && batch_rc != -ENODATA) {
    return batch_rc;
  }
  ...
  bufferlist v_bl;
  if (int rc = ioctx.getxattr(ext.soid, XATTR_VERSION, v_bl); rc < 0) {
    if (rc == -ENODATA) {
  ...
  if (int rc = init_missing_xattr(ext.soid, XATTR_EXCL, bufferlist()); rc < 0) return rc;
  if (int rc = init_missing_xattr(ext.soid, XATTR_STATE, state2bl(LockLevel::Shared)); rc < 0) return rc;
  if (int rc = init_missing_xattr(ext.soid, XATTR_WRITE_EPOCH, uint2bl(0)); rc < 0) return rc;
  if (int rc = init_missing_xattr(ext.soid, XATTR_SIZE, uint2bl(0)); rc < 0) return rc;
  if (int rc = init_missing_xattr(ext.soid, XATTR_ALLOCATED, uint2bl(0)); rc < 0) return rc;
  if (int rc = init_missing_xattr(ext.soid, XATTR_LAYOUT_STRIPE_UNIT, uint2bl(1)); rc < 0) return rc;
  if (int rc = init_missing_xattr(ext.soid, XATTR_LAYOUT_STRIPE_COUNT, uint2bl(1)); rc < 0) return rc;
  if (int rc = init_missing_xattr(ext.soid, XATTR_LAYOUT_OBJECT_SIZE, uint2bl(1<<object_size)); rc < 0) return rc;

  return 0;
}

int SimpleRADOSStriper::init_missing_xattr(const std::string& soid, const char* xattr_name, const ceph::bufferlist& default_bl)
{
  while (true) {
    ...
    read_op.stat(&size, &mtime, &stat_rval);
    read_op.getxattr(xattr_name, &bl, &xattr_rval);

    bufferlist out_bl;
    int rc = ioctx.operate(soid, &read_op, &out_bl);
    if (rc < 0 && rc != -ENODATA) {
      return rc;
    }

    if (rc == -ENODATA || xattr_rval == -ENODATA) {
      uint64_t obj_ver = ioctx.get_last_version();
      librados::ObjectWriteOperation write_op;
      write_op.assert_version(obj_ver);
      write_op.setxattr(xattr_name, default_bl);

      int write_rc = ioctx.operate(soid, &write_op);
      if (write_rc == -ERANGE || write_rc == -EOVERFLOW) {
        continue;
      }
      return write_rc;
    }
...
int SimpleRADOSStriper::lock(LockLevel ilock)
{
  ...
  while (locked < ilock) {
    ...
    auto op = librados::ObjectWriteOperation();
    int flags = (locked > None) ? LOCK_FLAG_MAY_RENEW : 0;
